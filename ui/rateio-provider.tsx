...
export interface RateioMutationContext {
    ...
    createShareLink: UseMutationResult<
        ...
        void
    >;
    revokeShareLink: UseMutationResult<
...
export default function RateioProvider({
    ...
    const addExpense = useMutation({
        mutationFn: (input: CreateManualExpenseInput) =>
    ...
    const changeStatus = useMutation({
        mutationFn: (status: RateioStatus) =>
            changeRateioStatus(rateioId, status),
        onSuccess: async (result) => {
            if (result.success) await invalidateRateio();
        },
    });
    ...
    const revokeShareLinkMutation = useMutation({
        mutationFn: (shareLinkId: string) =>
            revokeShareLink(rateioId, shareLinkId),
        onSuccess: async (result) => {
            if (result.success) {
                await Promise.all([
                    invalidateRateio(),
    ...
    useEffect(() => {
        ...
        socket.on("rateio.state", (payload) => {
            ...
            queryClient.setQueryData<RateioSessionData>(
                ...
                (current) =>
                    current
                        ? normalizeRealtimeSnapshot(parsed.data, current)
                        : current,
            );
        });
    ...
    return (
        <RateioContext.Provider value={value}>
            {children}
