...
export default function ManualExpenseDrawer({
  ...
  function buildAllocations(amountMinor: number): AllocationResult {
    if (selectedMemberIds.length === 0) {
      return { error: "Selecione pelo menos uma pessoa para dividir o item." };
    }
    ...
    const allocations = selectedMemberIds.map((memberId) => ({
      memberId,
      splitType,
      baseAmountMinor: parseMinorAmount(customAmounts[memberId] ?? "") ?? -1,
    }));
    if (
      allocations.some((allocation) => allocation.baseAmountMinor <= 0) ||
      allocations.reduce((sum, allocation) => sum + allocation.baseAmountMinor, 0) !==
        amountMinor
    ) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result: CreateManualExpenseResult = await createManualExpense(input);
      if (!result.success) {
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen} shouldScaleBackground={false}>
      <DrawerTrigger asChild>
        <Button className="w-full sm:w-auto" disabled={!isActive || members.length === 0}>
          + Adicionar item
        </Button>
      </DrawerTrigger>
      <DrawerContent className="max-h-[92dvh] overflow-y-auto p-6 sm:p-8">
        <div className="mx-auto w-full max-w-2xl">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <DrawerDescription className="mt-2">
                Registre um item em {baseCurrency} e escolha como dividir a conta.
              </DrawerDescription>
            </div>
            <DrawerClose asChild>
          ...
          <form className="mt-6 space-y-5" onSubmit={handleSubmit}>
            <div>
              <label htmlFor="expense-item-name">Nome do item</label>
              <input
            ...
            <div>
              <label htmlFor="expense-item-amount">Valor ({baseCurrency})</label>
              <input
                className={inputClassName()}
                inputMode="decimal"
                id="expense-item-amount"
                onChange={(event) => setAmount(formatInputAmount(event.target.value))}
                placeholder="0,00"
                required
                value={amount}
              />
              <p className="mt-1 text-xs text-muted-foreground">
                O valor é convertido para centavos antes de ser enviado.
              </p>
            </div>

            <div>
              <label htmlFor="expense-item-payer">Quem pagou?</label>
              <select
                className={inputClassName()}
            ...
            <div>
              <label htmlFor="expense-split-type">Forma de divisão</label>
              <select
                className={inputClassName()}
                ...
                value={splitType}
              >
                <option value="EQUAL">{splitLabel("EQUAL")}</option>
                <option value="PERCENTAGE">{splitLabel("PERCENTAGE")}</option>
                <option value="CUSTOM">{splitLabel("CUSTOM")}</option>
              </select>
            </div>
                ...
                <div className="mt-2 space-y-2">
                  {selectedMembers.map((member) => (
                    <div className="flex items-center gap-3" key={member.id}>
                      <label className="min-w-0 flex-1 truncate" htmlFor={`allocation-${member.id}`}>
                        {member.displayName}
                      </label>
                      <input
                        ...
                        inputMode="decimal"
                        onChange={(event) => {
                          const value = formatInputAmount(event.target.value);
                          if (splitType === "PERCENTAGE") {
                            setPercentages((current) => ({ ...current, [member.id]: value }));
                          } else {
