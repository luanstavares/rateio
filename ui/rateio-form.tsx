...
import { createRateio, type CreateRateioResult } from '../app/actions/rateios';
...
export default function RateioForm() {
  ...
  const createRateioMutation = useMutation<
    CreateRateioResult,
    Error,
    CreateRateioInput
  >({
    mutationFn: createRateio,
    onError: () => {
      setError('Não foi possível criar o rateio agora. Tente novamente.');
    },
    onSuccess: (result) => {
      if (!result.success) {
  ...
  const isPending = createRateioMutation.isPending;
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    createRateioMutation.mutate(input);
  }

  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      <div className="space-y-2">
        <label htmlFor="rateio-title" className="text-sm font-medium">
          Nome do rateio
        </label>
        <input
          id="rateio-title"
      ...
      <div className="space-y-2">
        <label htmlFor="rateio-description" className="text-sm font-medium">
