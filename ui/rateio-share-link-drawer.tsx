...
function LinkIcon() {
  return (
    ...
    >
      <path
        d="m10 13.5 4-4m-7.5 7.5-1.25 1.25a3.18 3.18 0 0 1-4.5-4.5L5 9.5a3.18 3.18 0 0 1 4.5 0M14.5 14.5a3.18 3.18 0 0 0 4.5 0l4.25-4.25a3.18 3.18 0 0 0-4.5-4.5L17.5 7"
        stroke="currentColor"
        strokeLinecap="round"
...
export default function RateioShareLinkDrawer({
  ...
  function loadLinks() {
    ...
    startTransition(async () => {
      const result: ShareLinkMetadataResult = await listShareLinks(rateioId);
      if (!result.success) {
  ...
  function handleCreate() {
    ...
    setCopyState("idle");
    startTransition(async () => {
      const result: CreateShareLinkResult = await createShareLink(rateioId);
      if (!result.success) {
        setError(result.error);
        return;
      }
  ...
  return (
    <Drawer open={open} onOpenChange={handleOpenChange}>
      ...
      <DrawerContent className="max-h-[90vh] overflow-y-auto p-6">
        ...
        <DrawerDescription>
          Crie um link para seus amigos entrarem neste rateio sem criar uma
          conta.
        </DrawerDescription>
        ...
        {activeLink ? (
          <div className="rounded-md border border-primary/50 bg-primary/5 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-sm font-semibold">Link atual</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  Criado em {formatCreatedAt(activeLink.createdAt)}
                </p>
              </div>
            ...
            {shareUrl ? (
              ...
              <p className="mt-4 text-sm text-muted-foreground">
                Por segurança, o link só fica disponível depois de criar ou
                regenerar.
              </p>
            )}
            <Button
          ...
          <div className="rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">
            Nenhum link ativo. Crie um link para compartilhar este rateio.
          </div>
        )}
