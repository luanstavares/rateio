      ...
      <div className="relative z-10 mx-auto flex w-full max-w-3xl flex-col items-center text-center lg:max-w-xl">
        <div className="flex flex-col items-center gap-3">
          <h1 className="text-4xl font-bold leading-tight sm:text-5xl md:text-6xl">
            {title}{' '}
            <Logo size="small" className="text-4xl sm:text-5xl md:text-6xl" />
          </h1>
        ...
        <div className="mt-12 flex w-full flex-col items-center justify-center gap-4">
          {options.map((option, index) => {
            if (index === 0) {
              return (
                <Button asChild key={option.title} className="min-w-44">
              ...
              return (
                <React.Fragment key={option.title}>
                  ...
                  <Button asChild className="min-w-60" variant="outline">
                    <Link href={option.href}>
                      {option.title}
                      {option.icon}
                    </Link>
