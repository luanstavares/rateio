...
function normalizeAllocation(value: unknown): RateioSessionAllocation {
  ...
  return {
    ...
    splitType:
      allocation.splitType === 'PERCENTAGE' || allocation.splitType === 'CUSTOM'
        ? allocation.splitType
        : 'EQUAL',
    percentageBasisPoints: typeof percentage === 'number' ? percentage : null,
...
function normalizeExpense(value: unknown): RateioSessionExpense {
  ...
  return {
    ...
    items: items.sort((left, right) => compareNewest(left, right)),
  };
}
...
function normalizeRateio(value: unknown): RateioSessionData['rateio'] {
  ...
  return {
    ...
    preferredPaymentRail: stringValue(rateio.preferredPaymentRail) ?? 'PIX',
...
function normalizeSession(
  ...
  const normalizedMembers = members.map(normalizeMember).sort(compareOldest);
...
export function isRateioSessionData(
  value: unknown,
): value is RateioSessionData {
